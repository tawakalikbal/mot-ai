    ...
    >
      <div className="container mx-auto flex justify-between items-center px-4 md:px-0"> {/* Tambah px-4 di mobile, hapus di md */}
        ...
        <div className="flex items-center space-x-6"> {/* Perbesar space-x-6 dari space-x-5 */}
          ...
          <div className="hidden md:flex items-center space-x-6"> {/* Perbesar space-x-6 dari space-x-5 */}
            {/* Language Dropdown */}
            <div className="relative group p-2 rounded-lg transition-colors duration-200
                            hover:bg-gray-100/10 dark:hover:bg-gray-800/20">
              <button
                onClick={() => toggleDropdown("language")}
                className={`flex items-center space-x-2 text-base cursor-pointer font-semibold
                            transition-all duration-300 transform group-hover:scale-105
                            ${isNavbarHovered ? "text-gray-700" : "text-white"}
                            `}
              >
                <span>GB-EN</span> {/* You might want to update this dynamically based on selected language */}
                <svg
              ...
              <ul
                className={`absolute right-0 mt-3 bg-white shadow-lg rounded-md overflow-hidden py-1
                  transition-all duration-300 ease-in-out transform z-20
                  ${isLanguageOpen ? "max-h-72 opacity-100" : "max-h-0 opacity-0 pointer-events-none"}
              ...
              >
                {languageData.map((countryGroup, index) => (
                  <React.Fragment key={countryGroup.country}>
                    <li>
                      <div className="flex items-center px-4 py-2 text-gray-500 text-sm font-semibold sticky top-0 bg-white z-10">
                    ...
                    {countryGroup.languages.map((lang) => (
                      <li key={lang.id}>
                        <a
                          className="block px-4 py-2 cursor-pointer text-gray-700 hover:bg-gray-100 transition-colors"
            ...
            >
              {/* Main Profile Circle (Blue) */}
              <div className="relative flex items-center justify-center w-11 h-11 rounded-full bg-blue-500
                              transition-all duration-300 transform group-hover:scale-105"
              >
              ...
              {/* Dropdown Arrow Circle - Hidden by default, visible on group hover */}
              <div className={`absolute -right-0 -bottom-0.5
                              flex items-center justify-center w-6 h-6 rounded-full bg-white shadow-md
                              transition-all duration-300 transform
                              ${isNavbarHovered ? "group-hover:scale-110" : "group-hover:scale-110"}
                              ${isNavbarHovered ? "text-gray-700" : "text-gray-700"}
              ...
              <ul
                className={`absolute mt-2 top-full right-0 bg-white shadow-lg rounded-md overflow-hidden py-1
                  transition-all duration-300 ease-in-out transform z-20
                  ${isProfileOpen ? "max-h-screen opacity-100" : "max-h-0 opacity-0 pointer-events-none"}
                  w-48
                `}
              >
